...
def cargar_datos():
    ...
    return gdf_medellin, df_customers, df_employees

# Generar coordenadas aleatorias dentro de Medellín
def generar_coordenadas(poligono):
...
def crear_datos(gdf_medellin, df_customers, df_employees, mu, sigma, order):
    longitude, latitude = generar_coordenadas(gdf_medellin.loc[0]['geometry'])
...
def enviar_datos(socket_cliente, gdf_medellin, df_customers, df_employees, mu, sigma, order_base):
    list_data = []
    for _ in range(random.randint(5, 10)):  # Genera más de 5 datos
        data = crear_datos(gdf_medellin, df_customers, df_employees, mu, sigma, order_base)
...
def ejecutar_servidor():
    gdf_medellin, df_customers, df_employees = cargar_datos()
    order_base = 'd8b9b417-b098-4344-b137-362894e4dccb'
    mu, sigma = 500, 175
    mi_socket = iniciar_servidor()
    while True:
        conexion, addr = mi_socket.accept()
        order_base = enviar_datos(conexion, gdf_medellin, df_customers, df_employees, mu, sigma, order_base)
